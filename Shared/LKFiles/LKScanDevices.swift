...
class LKScanDevices: LKDiscoveryProtocol
{
    ...
    func get(serialBase32: String?, lockMac: String?) -> LKScanResult?
    {
        _reachableDevicesQueue.sync {
            return _visibleDevices.first { (serialBase32 != nil && $0.lkCommDevice?.serial.serialBase32 == serialBase32)
                || (lockMac != nil && $0.ttLockCommDevice?.lockMac == lockMac) }
        }
    }
    ...
    private func _insertDevice(device: LKScanResult)
    {
        _reachableDevicesQueue.async {
            if let device = self._visibleDevices.first(where: {
                (device.serial != nil && $0.lkCommDevice?.serial.raw == device.serial?.raw)
                || (device.lockMac != nil && $0.ttLockCommDevice?.lockMac == device.lockMac) }) {
        ...
        _visibleTTLockTimer = Timer.scheduledTimer(withTimeInterval: 5.0, repeats: true) { _ in
            ...
            self._reachableDevicesQueue.async {
                self._visibleDevices.removeAll {
                    let isExpired = (Date().timeIntervalSince1970
                                     - ($0.ttLockCommDevice?.date.timeIntervalSince1970 ?? Double(0.00))) > 5
                    return $0.kind == .ttLock && isExpired
                }
            }
    ...
    {
        let knownLKDevices = self._visibleDevices.compactMap({  $0.lkCommDevice })
        let knownSerials = knownLKDevices.compactMap({ $0.serial.raw })

        if knownLKDevices != devices {
            let disappearedSerials =
                knownSerials.filter { known in !devices.contains(where: { $0.serial.raw == known }) }
            _reachableDevicesQueue.sync {
                self._visibleDevices.removeAll { disappearedSerials.contains($0.lkCommDevice?.serial.raw ?? Data()) }
            }
